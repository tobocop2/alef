...
fn test_pyi_plugin_protocol_widens_sequence_returns_but_not_params() {
    ...
    // PyO3 accepts any `PySequence` here, so `list[list[float]]` would reject NumPy backends.
    assert!(
    ...
    // `str` satisfies `Iterable[str]` but PyO3 rejects it, so widening here would delete a check.
    assert!(
